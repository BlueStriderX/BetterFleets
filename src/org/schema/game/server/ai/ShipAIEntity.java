...
public class ShipAIEntity extends SegmentControllerAIEntity<Ship> implements Transformable, HittableAIEntityState {
	...
	private float difficulty = ((Integer) ServerConfig.AI_WEAPON_AIMING_ACCURACY.getCurrentState()).floatValue();
	...
	private void check(UsableControllableFireingElementManager<? extends FireingUnit<?, ?, ?>, ? extends ControlBlockElementCollectionManager<? extends FireingUnit<?, ?, ?>, ?, ?>, ?> em, short controllerType, Range shootingRange) {
		...
		if (em.getCollectionManagers().isEmpty()) {
			...
			for (int i = 0; i < em.getCollectionManagers().size(); i++) {
				ControlBlockElementCollectionManager<? extends FireingUnit<?, ?, ?>, ?, ?> m = em.getCollectionManagers().get(i);
				...
				if (controlling) {
					...
					if (m.getSlaveConnectedElement() != Long.MIN_VALUE) {
						ManagerModuleCollection<?, ?, ?> managerModuleCollection = ((ManagedSegmentController<?>) m.getSegmentController()
						).getManagerContainer().getModulesControllerMap().get((short) ElementCollection.getType(m.getSlaveConnectedElement()));
					...
					for (FireingUnit<?, ?, ?> c : m.getElementCollections()) {
						if (supportCol != null && ((Combinable) em).getAddOn() != null) {
							Modifier mod = (((Combinable) em).getAddOn()).getGUI(m, c, supportCol, null);
							if (mod instanceof WeaponUnitModifier) {
								WeaponUnitModifier gui = (WeaponUnitModifier) (((Combinable) em).getAddOn()).getGUI(m, c, supportCol, null);
	...
	private static class Range{
		...
		public void checkSanity() {
			if(range <= 0 || range > Float.POSITIVE_INFINITY) {
				range = 1500;
			}

			range *= (Float)ServerConfig.AI_ENGAGEMENT_RANGE_OF_MIN_WEAPON_RANGE.getCurrentState();
		}
	}

	private void checkShootingRange(Timer timer) {
		if (timer.currentTime - lastShootingRangeCheck > 5000) {
			shootingRange.mode = Range.Mode.MIN;
	...
	public void updateAIServer(Timer timer) throws FSMException {
		if (getEntity().getDockingController().isDocked() && getEntity().getDockingController().isTurretDocking()) {
			...
			if (currentState.equals("Ship")) {
				...
				((AIConfiguationElements<String>) (getEntity()).getAiConfiguration().get(Types.TYPE))
						.setCurrentState("Turret", true);
				getEntity().getAiConfiguration().applyServerSettings();

				System.err.println("[AI] Auto Set " + getEntity()
				                   + " from "+currentState+" mode to Turret mode because it's docked");
			}
		} else if (isTurretDockedLastAxis() &&
		           !getEntity().getAiConfiguration().get(Types.TYPE).getCurrentState()
		                       .equals("Turret")) {
		...
		if (!getEntity().isDocked() && !getEntity().isConrolledByActivePlayer()) {
			ScanAddOn s = getEntity().getManagerContainer().getScanAddOn();
			if (s.isCharged() && !s.isActive() && s.canExecute()) {
				s.executeModule();
			}
		}
	...
	private void updateAIServerInCombat(Timer timer) {
		StealthAddOn a = getEntity().getManagerContainer().getStealthAddOn();
		if(a.isCharged() && !a.isActive()) {
			a.handleControl(unit, timer);
		}
	...
	public void updateAIClient(Timer timer) {
		...
		if(moveDir.equals(FleetBreaking.breakMoving)) {
			...
			if (getEntity().getNetworkObject().targetId.get() > 0) {
	...
	public boolean isTurretDockedLastAxis(){
		...
		return (getEntity().railController.isDockedAndExecuted() && getEntity().railController.isTurretDocked() &&
		        getEntity().railController.isTurretDockLastAxis()) || getEntity().getDockingController().isDocked();
	}
	@Override
	...
	public boolean isBigEvading(){
		return System.currentTimeMillis() - evadingTime < getEvadingDuration();
	}
	...
	public void updateGeneral(Timer timer){
		if(isOnServer()){
			...
			if(!isTurretDockedLastAxis() && getEntity().isInFleet() && !((AIConfiguationElements<String>) (getEntity()).getAiConfiguration().get(Types.TYPE)).getCurrentState().equals("Fleet")){
				...
				((AIConfiguationElements<String>) (getEntity()).getAiConfiguration().get(Types.TYPE))
						.setCurrentState("Fleet", true);
				((AIConfiguationElements<Boolean>) (getEntity()).getAiConfiguration().get(Types.ACTIVE))
	...
	public void updateOnActive(Timer timer) throws FSMException {
		...
		if(!isTurretDockedLastAxis() && !getEntity().isInFleet() && !((AIConfiguationElements<String>) (getEntity()).getAiConfiguration().get(Types.TYPE)).getCurrentState().equals("Ship")){
			...
			((AIConfiguationElements<String>) (getEntity()).getAiConfiguration().get(Types.TYPE))
					.setCurrentState("Ship", ((AIConfiguationElements<String>) (getEntity()).getAiConfiguration().get(Types.ACTIVE)).isOn());
			if(isOnServer()) {
				getEntity().getAiConfiguration().applyServerSettings();
			}
		}
	...
	public void handleHitBy(float actualDamage, Damager from) {
		if (getCurrentProgram() != null && getCurrentProgram() instanceof TargetProgram<?> && getState().getUpdateTime() - lastAggroTest > 25000) {
			...
			if ((a.getCurrentState().equals("Any") ||
			     ...
			     (a.getCurrentState().equals("Stations") && (from instanceof SpaceStation || from instanceof Planet))
			)) {
	...
	public void moveTo(Timer timer, final Vector3f toDir, boolean orientate) {
		...
		float updateFrequency = ThrusterElementManager.getUpdateFrequency(); //one udpate for every 30 ms
		...
		timeTracker = Math.min(updateFrequency * 100f, timeTracker); //make sure it's save against long lags
	...
	private void iterateTurretOrientation(Vector3f oForce, Quat4f toDir, float timeStep){
		...
		float turretOrientateSpeed = Math.max(
				VoidElementManager.AI_TURRET_ORIENTATION_SPEED_MIN,
				Math.min(VoidElementManager.AI_TURRET_ORIENTATION_SPEED_MAX,
				         VoidElementManager.AI_TURRET_ORIENTATION_SPEED_DIV_BY_MASS / Math.max(0.01f, getEntity().getTotalPhysicalMass())));
		...
		getEntity().getPhysics().orientate(getEntity(),
		                                   ...
		                                   oForce.x, oForce.y, oForce.z, timeStep);
	}
	public void orientate(Timer timer, Quat4f toDir) {
		...
		if (getEntity().railController.isDockedOrDirty()) {
			...
			getEntity().getPhysics().orientate(getEntity(),
			                                   ...
			                                   GlUtil.getUpVector(new Vector3f(), getWorldTransform()),
			                                   GlUtil.getRightVector(new Vector3f(), getWorldTransform()),
			                                   oForce.x, oForce.y, oForce.z, timer.getDelta());
			...
			if(toDir.w == 0){
				...
				getEntity().getPhysics().orientate(getEntity(),
				                                   forward, up, right, oForce.x*1.5f, oForce.y*1.5f, oForce.z*1.5f, timer.getDelta());
			}else{
				...
				getEntity().getPhysics().orientate(getEntity(),
				                                   GlUtil.getForwardVector(new Vector3f(), m),
				                                   GlUtil.getUpVector(new Vector3f(), m),
				                                   GlUtil.getRightVector(new Vector3f(), m),
				                                   oForce.x, oForce.y, oForce.z, timer.getDelta());
			}
		}
	}
	...
	public String toString() {
		return super.toString() + "; WEP-Range: " + getShootingRange()+" salv-Range: "+getSalvageRange()+"; "+
		       (program != null && program.getMachine() != null && getStateCurrent() != null ?
		        getStateCurrent().getDescString() : "");
	}
	...
	public void doShooting(AIControllerStateUnit<?> unit, Timer timer) {
		...
		try {
			if(getEntity().getNetworkObject().targetType.getByte() == SimpleGameObject.MINABLE){
				...
				((AIShipControllerStateUnit)unit).timesBothMouseButtonsDown = 0;
				((AIShipControllerStateUnit)unit).useBothMouseButtonsDown = false;
				RepairPasteFabricatorSystem supportSystem = FleetUtils.getSupportSystem(getEntity());
				if(supportSystem != null && GameCommon.getGameState().getFactionManager().getRelation(unit.getEntity().getFactionId(), getEntity().getFactionId()).equals(FactionRelation.RType.FRIEND) && !(getEntity().getDockingController().getRoot().equals(((TargetProgram<?>) getAIConfig().getAiEntityState().getCurrentProgram()).getTarget()))) supportSystem.handle(timer);
				else {
					if(getEntity().getDockingController().isDocked() || getEntity().getDockingController().isTurretDocking()){
