...
/**
 * Modified version of FleetManager.
 *
    ...
    private void checkChacheStatus() {
        ...
        for(Fleet f : fleetCache.values()){
            ...
            if(updateActionMapCounter.get(f.dbid) > 0){
                ...
                //there is unfinished buissness for this fleet
                continue;
            }
...
//		System.err.println("[SERVER][FLEET] Executing command: "+t.name()+" -> "+t.transition.name()+"; on Fleet: "+fleet);
    ...
    private Fleet getCachedByEntity(SegmentController c){
        ...
        if(c.dbId < 0){
            ...
            return ((id = fleetsByEntityDbId.get(c.dbId)) > 0) ? fleetCache.get(id) : null;
        }
    }
    ...
    public void onJoinedPlayer(PlayerState c) {
        ...
        FactionManager factionManager = ((FactionState)c.getState()).getFactionManager();
...
//				}
    ...
    public Collection<Fleet> getAvailableFleetsClient() {
        ...
        ObjectArrayList<Fleet> fleets = new ObjectArrayList<Fleet>();
...
//			System.err.println("[FLEET] "+state+" UNCACHING FLEET "+fltToUncache);
            ...
            case MISSION_UPDATE:
                ...
                break;
            case REMOVE_MEMBER:
                System.err.println("[FLEET] REMOVING MEMEBER REQUEST (EXEC ON SERVER): "+mod);
                ...
                if(fltToDelMemFrom != null){
                    ...
                    if(removed != null){
                        uncacheMember(removed.UID, removed.entityDbId);
                        if(removed.isLoaded()){
                            ((AIConfiguationElements<Boolean>) ((Ship)removed.getLoaded()).getAiConfiguration().get(Types.ACTIVE))
    ...
    public void requestShipAdd(Fleet fleet,
                               SegmentController c) {
    ...
    public void requestFleetRemove(Fleet f) {
        if(!isOnServer() && !f.getOwner().toLowerCase(Locale.ENGLISH).equals(((GameClientState)state).getPlayer().getName().toLowerCase(Locale.ENGLISH))){
...
//		System.err.println("UPDATING :::::::::: "+c+" -> "+nw );
    ...
    public void submitTargetPositionToClients(Fleet fleet) {
        ...
        if(fleet.getCurrentMoveTarget() != null){
            mod.target = new Vector3i(fleet.getCurrentMoveTarget());
        }else{
            mod.target = null;
        }
        sendMod(mod);
