...
/**
 * Modified version of HudContextHelpManager.
 *
...
public class HudContextHelpManager {
    ...
    private void drawLeft(List<HudContextHelperContainer> q) {
        if(q.isEmpty()){
            setLeftStartPosY(GLFrame.getHeight());
            setLeftEndPosY(GLFrame.getHeight());
        } else {
            setLeftStartPosY(GLFrame.getHeight() - bottomOffsetY);
        }

        int x = 0;
        int y = getLeftStartPosY();
    ...
    //INSERTED CODE
    public List<HudContextHelperContainer> queueMouse = new ObjectArrayList<HudContextHelperContainer>();
    public List<HudContextHelperContainer> queueLeft = new ObjectArrayList<HudContextHelperContainer>();
    public List<HudContextHelperContainer> queueBlock = new ObjectArrayList<HudContextHelperContainer>();

    public int getMouseYHeight() {
        int mouseYHeight = 0;
    ...
    public HudContextHelperContainer addHelper(InputType type, int key, Object text, Hos position, ContextFilter filter){
        try {
            ...
            if (((ContextGroup) EngineSettings.G_ICON_CONTEXT_FILTER.getCurrentState()).containsFilter(filter)) {
    ...
    private void addBuildAndTake(){
        if(currentPiece != null){
            ...
            if(slot == null){
                addHelper(InputType.MOUSE, !EngineSettings.C_MOUSE_BUTTON_SWITCH.isOn() ? 1 : 0, Lng.str("Remove"), Hos.MOUSE, ContextFilter.IMPORTANT);
            }else if(slot.isMultiSlot() || !slot.isMetaItem()){
                addHelper(InputType.MOUSE, !EngineSettings.C_MOUSE_BUTTON_SWITCH.isOn() ? 0 : 1, Lng.str("Place"), Hos.MOUSE, ContextFilter.IMPORTANT);
                addHelper(InputType.MOUSE, !EngineSettings.C_MOUSE_BUTTON_SWITCH.isOn() ? 1 : 0, Lng.str("Remove"), Hos.MOUSE, ContextFilter.IMPORTANT);
            }
        }
    ...
    private boolean isSelectable(ElementInformation currentInfo){
        return
                ElementKeyMap.getFactorykeyset().contains(currentInfo.getId()) ||
                        currentInfo.getId() == ElementKeyMap.STASH_ELEMENT ||
                        currentInfo.getId() == ElementKeyMap.SHIPYARD_COMPUTER ||
    ...
    private void addBlockOption(){
        ...
        } else if (currentPiece != null && currentPiece.getSegmentController() instanceof ShopSpaceStation && currentPiece.equalsPos(WorldCreatorShopMannedFactory.shopkeepSpawner)) {
    ...
    private void addFlightMode(Timer timer) {
        ...
        if(!getState().getPlayer().getCockpit().isCore()) {
        ...
        if(getState().getPlayer().getCockpit().isInCockpitAdjustment()) {
        ...
        Ship s = getState().getShip();
        if(s != null) {
            for (ControllerStateUnit u : getState().getPlayer().getControllerState().getUnits()) {
            ...
            if(manModCol != null && manModCol.getElementManager() instanceof UsableCombinableControllableElementManager<?,?,?,?>) {
                ...
                if(cm != null && cm instanceof DamageDealer) {
                    ...
                    for(int i = 0; i < InterEffectSet.length; i++) {
                        ...
                        addInfo(Hos.MOUSE, ContextFilter.CRUCIAL, t.shortName.getName(t)+": "+Math.round((attackSet.getStrength(t)/3.0f)*100f)+"%");
                    }
                }
            }
