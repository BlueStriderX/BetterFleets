...
/**
 * Modfied version of PlayerPanel.
 *
...
public class PlayerPanel extends GUIElement {
    ...
    private boolean catalogControllerManagerActive() {
        return getState().getGlobalGameControlManager().getIngameControlManager().
                getPlayerGameControlManager().getCatalogControlManager().isTreeActive();
    }
    private boolean fleetControllerManagerActive() {
        return getState().getGlobalGameControlManager().getIngameControlManager().
                getPlayerGameControlManager().getFleetControlManager().isTreeActive();
    }
    ...
    public void draw() {
        ...
        topBar.draw();
        if (inventoryActive() || shopActive()) {
            getBuildSideBar().activateDragging(true);
        } else {
            getBuildSideBar().activateDragging(false);
        }
...
//				assert(false):e.getType()+"; "+e.getCount(true);
        ...
        //
        this.setPanelActive(panelActiveTest);
    ...
    public void onInit() {
        ...
        shipInformationPanel = new ShipInformationPanel(getState());
        shipInformationPanel.onInit();
        ...
        close = new GUIAncor(getState(), 39, 26);
        ...
        inventoryTab = new GUIAncor(getState(), 147, 40);
        ...
        aiMenuTab = new GUIAncor(getState(), 147, 40);
        ...
        factionMenuTab = new GUIAncor(getState(), 147, 40);
        ...
        structureMenuTab = new GUIAncor(getState(), 147, 40);
        ...
        weaponTab = new GUIAncor(getState(), 147, 40);
        ...
        catalogTab = new GUIAncor(getState(), 147, 40);
        ...
        shopTab = new GUIAncor(getState(), 147, 40);
        ...
        naviTab = new GUIAncor(getState(), 147, 40);
        ...
        infoTextOverlay = new GUITextOverlay(10, 10, getState());
        ...
        if (GuiDrawer.isNewHud()) {
            ...
            setBuildSideBar(new BottomBarBuild(getState(), inventoryPanelNew));
        } else {
            setBuildSideBar(new BuildSideBarOld(getState()));
        }
        getBuildSideBar().onInit();
    ...
    private boolean structureControllerManagerActive() {
        return getState().getGlobalGameControlManager().getIngameControlManager().
                getPlayerGameControlManager().getStructureControlManager().isTreeActive();
    }
    private boolean thrustManagerManagerActive() {
        return getState().getGlobalGameControlManager().getIngameControlManager().
                getPlayerGameControlManager().getThrustManager().isTreeActive();
    }
    ...
    public void doOrientation() {
        shipInformationPanel.orientate(GUIElement.ORIENTATION_LEFT | GUIElement.ORIENTATION_VERTICAL_MIDDLE);

        getBuildSideBar().orientate(GUIElement.ORIENTATION_HORIZONTAL_MIDDLE | GUIElement.ORIENTATION_BOTTOM);
...
//			System.err.println("DRLKDLKJDLKJ");
    ...
    public void drawToolTips() {
        ...
        if (inGameActive()) {
            GUIElement.enableOrthogonal();
            ((TooltipProvider) getBuildSideBar()).drawToolTip();
            GUIElement.disableOrthogonal();
    ...
    private boolean factionControllerManagerActive() {
        return getState().getGlobalGameControlManager().getIngameControlManager().
                getPlayerGameControlManager().getFactionControlManager().isTreeActive();
    }

    public boolean isInAstronautMode() {
        return getState().getGlobalGameControlManager().getIngameControlManager().
                getPlayerGameControlManager().getPlayerIntercationManager().getPlayerCharacterManager().isTreeActive();
    }
    ...
    public boolean isMouseOnPanel() {
        boolean onBuildPanel = PlayerInteractionControlManager.isAdvancedBuildMode(getState()) &&
                advancedBuildMode.isInside() ;
    ...
    private class TabCallback implements GUICallback {
        ...
        public void callback(GUIElement callingGuiElement, MouseEvent event) {
            if (event.getEventButtonState() && event.getEventButton() == 0) {
