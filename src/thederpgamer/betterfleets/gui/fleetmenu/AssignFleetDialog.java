 ...
 */
public class AssignFleetDialog extends GUIInputDialog {

	private final DeploymentsScrollableList deploymentsList;
	private final FleetDeploymentData deploymentData;

	public AssignFleetDialog(DeploymentsScrollableList deploymentsList, FleetDeploymentData deploymentData) {
			...
			switch((String) guiElement.getUserPointer()) {
				case "OK":
					if(getInputPanel().fleetList.getSelectedRow() != null && getInputPanel().fleetList.getSelectedRow().f != null) {
						FleetDeploymentManager.assignFleet(deploymentData, getInputPanel().fleetList.getSelectedRow().f);
	...
	public static class AssignFleetPanel extends GUIInputDialogPanel {
		...
		public AssignFleetScrollableList fleetList;
		private final FleetDeploymentData deploymentData;
		...
		public void onInit() {
			super.onInit();
			(fleetList = new AssignFleetScrollableList(getState(), getWidth(), getHeight(), getContent(), deploymentData)).onInit();
